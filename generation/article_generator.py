...
文章结构（必须严格遵循，这是硬性要求）：
1. 标题：必须重新生成，不直接复制上述"热点标题"或任一来源标题；标题中的引号、书名号、括号必须成对闭合。
...
- 禁止写成多个来源的摘要拼接
- 禁止复述来源标题
- 标题必须重新组织表达，不能照搬、近似照搬或只删改标点复制热点标题/来源标题；不得出现未闭合的引号、书名号或括号。
- 禁止空泛评价，如"具有重要意义""值得深思"，除非紧接着说明具体原因
- 分析必须克制，不得虚构新事实、处罚金额、人数、伤亡、官方结论
- 标题和小标题必须精确表达来源事实；如果来源只是手机、设备或物品坠落，不得写成“坠机”“空难”“飞机失事”或其他航空事故。
- 数字量级必须与来源一致；来源为“1.1千米/公里”时不得写成“万米高空”。
- 不得补写来源未提供的产品材料、技术原理、公司宣传语或功能机制；例如只可写“通过 Find My 定位”，不要自行解释为卫星信号、材料结构或实验室结论。
- 尽量不要使用“一是/二是/其一/其二/第一/第二”这类列表衔接词；如必须使用，必须成组完整出现，否则改成自然句。
- {target_body_text}
...
8. 标题必须重新组织表达，不得直接复制热点标题、来源标题或首稿标题；标题中的引号、书名号、括号必须成对闭合。"""
...
8. 不得使用固定套话凑字；
9. 只使用事实卡与已提供来源中的事实；
10. 标题必须重新组织表达，不得直接复制热点标题或来源标题；标题中的引号、书名号、括号必须成对闭合。
...
7. 标题必须重新组织表达，不能照搬、近似照搬或只删改标点复制热点标题/来源标题；标题中的引号、书名号、括号必须成对闭合。
8. 标题和小标题必须精确表达来源事实；如果来源只是手机、设备或物品坠落，不得写成“坠机”“空难”“飞机失事”或其他航空事故。
9. 数字量级必须与来源一致；来源为“1.1千米/公里”时不得写成“万米高空”。
10. 不得补写来源未提供的产品材料、技术原理、公司宣传语或功能机制；例如只可写“通过 Find My 定位”，不要自行解释为卫星信号、材料结构或实验室结论。
11. 尽量不要使用“一是/二是/其一/其二/第一/第二”这类列表衔接词；如必须使用，必须成组完整出现，否则改成自然句。
